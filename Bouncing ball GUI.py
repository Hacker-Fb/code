                ...
                try:
                    if pos[2] >= brick_pos[0] and pos[0] <= brick_pos[2]:
                        if pos[3] >= brick_pos[1] and pos[1] <= brick_pos[3]:
                            canvas.bell()
                            self.hit += 1
                            self.score.configure(text="Sudhanshu     Score: " + str(self.hit))
...
def start_game(event):
    ...
    if playing is False:
        playing = True
        score.configure(text="Sudhanshu     Score: 00")
        canvas.delete("all")
        ...
        while 1:
            if paddle.pausec !=1:
                ...
                if not ball.bottom_hit:
                    ...
                    canvas.create_text(250, 250, text="GAME OVER!\nGAME KAISA LAGA..\nSUDHANSHU KO BATAO!", fill="red", font="Consolas 24 ")
                    root.update_idletasks()
                    root.update()
                    playing = False
                    break
                ...
                try:
                    ...
                    m=canvas.create_text(250, 250, text="PAUSE KYUN KAR DIYA !!", fill="red", font="Consolas 24 ")
                root.update_idletasks()
                root.update()
